        ...
        <li key={i}>
          <div className='list-container'>
            <ul
              className='list'
              id={`list${i}`}
              onClick={event => this.handlehide(event, i)}
            >
              <li>
                {this.state.stocks[i]['symbol']}:{' '}
                {this.state.stocks[i]['companyName']}
              </li>
              <li>{this.state.stocks[i]['latestPrice']}</li>
              <li
                style={{
                  color:
                    this.state.stocks[i]['changePercent'] < 0 ? 'red' : 'green'
                }}
              >
                {(this.state.stocks[i]['changePercent'] * 100).toFixed(2)}%
              </li>
              <li>
